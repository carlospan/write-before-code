...
Process rules live in `content/`; hosts only differ by **where** the skill folder is installed and **how** you invoke it.
...
## What stays the same on every host
...
## Not a full host rewrite

We do **not** ship separate prompt trees per IDE. One methodology corpus + one entry skill keeps EN/ZH parity maintainable. If a host cannot load skills at all, fall back to pasting `SKILL.md` + pointing the agent at `content/<lang>/how-to/agent-prompts/00-overview.md`.
